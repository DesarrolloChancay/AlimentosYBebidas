...
class Establecimiento(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    tipo_establecimiento_id = db.Column(db.Integer, db.ForeignKey('tipos_establecimiento.id'))
    ...
    tipo_establecimiento = db.relationship('TipoEstablecimiento', backref='establecimientos', lazy=True)
    encargados = db.relationship('EncargadoEstablecimiento', backref='establecimiento', lazy=True)
    items_evaluacion = db.relationship('ItemEvaluacionEstablecimiento', backref='establecimiento', lazy=True)
...
class EncargadoEstablecimiento(db.Model):
    ...
    usuario = db.relationship('Usuario', backref='encargos', lazy=True)
...
class ItemEvaluacionBase(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    categoria_id = db.Column(db.Integer, db.ForeignKey('categorias_evaluacion.id'), nullable=False)
    codigo = db.Column(db.String(20), nullable=False)
    descripcion = db.Column(db.Text, nullable=False)
    riesgo = db.Column(db.Enum('Menor', 'Mayor', 'Crítico'), nullable=False, default='Menor')
...
class ItemEvaluacionEstablecimiento(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    establecimiento_id = db.Column(db.Integer, db.ForeignKey('establecimientos.id'), nullable=False)
    item_base_id = db.Column(db.Integer, db.ForeignKey('items_evaluacion_base.id'), nullable=False)
    descripcion_personalizada = db.Column(db.Text)
    factor_ajuste = db.Column(db.Numeric(3,2), default=1.00)  # Nuevo campo para ajuste de puntaje
    activo = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.TIMESTAMP, default=datetime.utcnow)
...
class Inspeccion(db.Model):
    ...
    # Relaciones usando strings para evitar importaciones circulares
    inspector = db.relationship('Usuario', foreign_keys=[inspector_id], backref='inspecciones_como_inspector', lazy=True)
    encargado = db.relationship('Usuario', foreign_keys=[encargado_id], backref='inspecciones_como_encargado', lazy=True)
...
class InspeccionDetalle(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    inspeccion_id = db.Column(db.Integer, db.ForeignKey('inspecciones.id'), nullable=False)
    item_establecimiento_id = db.Column(db.Integer, db.ForeignKey('items_evaluacion_establecimiento.id'), nullable=False)
...
class PlanSemanal(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('establecimiento_id', 'semana', 'ano'),)
